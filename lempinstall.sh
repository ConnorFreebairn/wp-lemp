...
# Update packages list and isntall fail2ban, ufw
apt-get update; apt-get install -y fail2ban ufw;

# Allow SSH, HTTP and HTTPS through firewall
ufw allow 22
ufw allow 80
ufw allow 443
...
apt-get update; apt-get install -y build-essential curl nano wget lftp unzip bzip2 arj nomarch lzop htop openssl gcc git binutils libmcrypt4 libpcre3-dev make python3 python3-pip supervisor unattended-upgrades whois zsh imagemagick uuid-runtime net-tools nginx
